...
class WNException : public util::RuntimeException {
public:

    WNException(std::string in_msg) : RuntimeException(in_msg) {
    };
};
...
#define DICTDIR         "\\dict"
#ifndef DEFAULTPATH
#define DEFAULTPATH	"C:\\users\\slate\\My Documents\\Visual Studio 2010\\Projects\\CBICA\\3rdParty\\WordNet-3.0\\dict"
#endif
...
public:
    ...
    unsigned int is_defined(const char *, int);

    /* Set bit for each POS that search word is in.  0 returned if
    word is not in WordNet. */
    unsigned int in_wn(char *, int);

    // Get how specific this word is (using WordNet average hypernym depth)
    float GetSpecificity(long offset);
    ...
    IndexPtr getindex(const char *, int);
...
private:
    //	char *wnrelease;		/* WordNet release/version number */

    //	char **lexfiles;	/* names of lexicographer files */
    //	char **ptrtyp;		/* pointer characters */
    //	char **partnames;	/* POS strings */
    //	char *partchars;	/* single chars for each POS */
    //	char **adjclass;	/* adjective class strings */
    //	char **frametext;	/* text of verb frames */


    /* File pointers for database files */

    int OpenDB; /* if non-zero, database file are open */
    ...
    *vidxfilefp, *vsentfilefp;
    ...
    int prflag, sense, prlexid;
    int overflag; /* set when output buffer overflows */
    char searchbuffer[SEARCHBUF];
    int lastholomero; /* keep track of last holo/meronym printed */
    char tmpbuf[TMPBUFSIZE]; /* general purpose printing buffer */
    char wdbuf[WORDBUF]; /* general purpose word buffer */
    char msgbuf[256]; /* buffer for constructing error messages */
    int adj_marker;
    char linebuf[LINEBUF];
    long last_bin_search_offset;


    void WNOverview(char *, int);
    void findverbgroups(IndexPtr);
    void add_relatives(int, IndexPtr, int, int);
    static void free_rellist(void);
    void printsynset(char *, SynsetPtr, char *, int, int, int, int);
    void printantsynset(SynsetPtr, char *, int, int);
    char *printant(int dbase, SynsetPtr synptr, int wdnum, char *templ, char *tail);
    void printbuffer(char *);
    void printsns(SynsetPtr, int);
    void printsense(SynsetPtr, int);
    void catword(char *buf, int bufsize, SynsetPtr, int, int, int);
    void printspaces(int, int);
    void printrelatives(IndexPtr, int);
    int HasHoloMero(IndexPtr, int);
    static int HasPtr(SynsetPtr, int);
    int getsearchsense(SynsetPtr, int);
    int depthcheck(int, SynsetPtr);
    static void interface_doevents();
    void getexample(char *, char *);
    int findexample(SynsetPtr);
    void traceptrs(SynsetPtr synptr, int ptrtyp, int dbase, int depth);
    void tracecoords(SynsetPtr synptr, int ptrtyp, int dbase, int depth);
    void traceclassif(SynsetPtr synptr, int dbase, int search);
    void tracenomins(SynsetPtr synptr, int dbase);
    void partsall(SynsetPtr synptr, int ptrtyp);
    void traceinherit(SynsetPtr synptr, int ptrbase, int dbase, int depth);
    void traceadjant(SynsetPtr synptr);
    void printframe(SynsetPtr synptr, int prsynset);
    void printseealso(SynsetPtr synptr);
    void freq_word(IndexPtr index);
    void wngrep(char *word_passed, int pos);
    void relatives(IndexPtr idx, int dbase);
    struct relgrp *mkrellist(void);

    // Morph functions

    void do_init(const char *wn_dir);
    static int strend(char *, char *);
    static char *wordbase(char *, int);
    static int hasprep(char *, int);
    char *exc_lookup(char *, int);
    char *morphprep(char *);
    char *deadjify(char *word);

    // Util functions
    void FreeSenseIndex(SnsIndexPtr snsidx);

    // Binsearch functions
    int bin_search_key(char *searchkey, char *line, int linelen, FILE *fp);
    ...
    int fnflag; /* if set, print lex filename after sense */
    int dflag; /* if set, print definitional glosses */
    int saflag; /* if set, print SEE ALSO pointers */
    int fileinfoflag; /* if set, print lex file info on synsets */
    int frflag; /* if set, print verb frames */
    int abortsearch; /* if set, stop search algorithm */
    int offsetflag; /* if set, print byte offset of each synset */
    int wnsnsflag; /* if set, print WN sense # for each word */
    static const char *wnrelease;
    ...
    static const char partchars[];
    static const char *adjclass[];

    /* Text of verb sentence frames */

    static const char *frametext[];
